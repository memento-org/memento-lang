...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (!workspaceFolder) {
    vscode.window.showErrorMessage('Memento LSP requires a workspace folder to be opened');
    return;
  }
  ...
  const testCommand = vscode.commands.registerCommand('memento.test', () => {
    vscode.window.showInformationMessage('Memento LSP Test - check output channel');
