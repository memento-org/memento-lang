...
instance {-# OVERLAPPING #-} Extractive h1 (h1 :*: h2) where
  hExtract :: (h1 :*: h2) f a -> h1 f a
  hExtract (h1 :*: _) = h1

instance {-# OVERLAPPABLE #-} (Extractive h1 h2) => Extractive h1 (h3 :*: h2) where
...
class Constructive (h1 :: (k -> Type) -> k -> Type) h2 h3 where
  hConstruct :: forall (f :: k -> Type) a. h1 f a -> h2 f a -> h3 f a

instance {-# OVERLAPPING #-} Constructive h1 h2 (h1 :*: h2) where
  hConstruct :: h1 f a -> h2 f a -> (h1 :*: h2) f a
  hConstruct h1 h2 = h1 :*: h2

instance {-# OVERLAPPABLE #-} (Constructive h1 h2 h3) => Constructive h1 (h4 :*: h2) (h4 :*: h3) where
